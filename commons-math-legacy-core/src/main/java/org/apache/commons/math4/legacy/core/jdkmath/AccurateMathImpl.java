 ...
 * @since 2.2
 */
public final class AccurateMathImpl {
    ...
    public static double acosh(final double a) {
        return AccurateMathImpl.log(a + AccurateMathImpl.sqrt(a * a - 1));
    }
    ...
    public static double asinh(double a) {
        ...
        double absAsinh;
        if (a > 0.167) {
            absAsinh = AccurateMathImpl.log(AccurateMathImpl.sqrt(a * a + 1) + a);
        } else {
            final double a2 = a * a;
            if (a > 0.097) {
    ...
    public static double atanh(double a) {
        ...
        double absAtanh;
        if (a > 0.15) {
            absAtanh = 0.5 * AccurateMathImpl.log((1 + a) / (1 - a));
        } else {
            final double a2 = a * a;
            if (a > 0.087) {
                ...
                if (yRawExp >= 1023) {
                    ...
                    if (yRawExp < 1075) {
                        ...
                        if ((yFullMantissa & integralMask) == yFullMantissa) {
                            ...
                            return AccurateMathImpl.pow(x, (y < 0) ? -l : l);
                        }
                    } else {
        ...
        private Split pow(final long e) {
            ...
            if (Double.isNaN(result.full)) {
                if (Double.isNaN(full)) {
                    ...
                    if (AccurateMathImpl.abs(full) < 1) {
                        return new Split(AccurateMathImpl.copySign(0.0, full), 0.0);
    ...
    public static double IEEEremainder(final double dividend, final double divisor) {
        if (getExponent(dividend) == 1024 || getExponent(divisor) == 1024 || divisor == 0.0) {
            ...
            final double n         = AccurateMathImpl.rint(dividend / divisor);
            final double remainder = Double.isInfinite(n) ? 0.0 : dividend - divisor * n;
            return (remainder == 0) ? AccurateMathImpl.copySign(remainder, dividend) : remainder;
        }
    }
    ...
    private static class ExpIntTable {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                EXP_INT_TABLE_A = new double[AccurateMathImpl.EXP_INT_TABLE_LEN];
                EXP_INT_TABLE_B = new double[AccurateMathImpl.EXP_INT_TABLE_LEN];

                final double[] tmp = new double[2];
                final double[] recip = new double[2];

                // Populate expIntTable
                for (int i = 0; i < AccurateMathImpl.EXP_INT_TABLE_MAX_INDEX; i++) {
                    AccurateMathCalc.expint(i, tmp);
                    EXP_INT_TABLE_A[i + AccurateMathImpl.EXP_INT_TABLE_MAX_INDEX] = tmp[0];
                    EXP_INT_TABLE_B[i + AccurateMathImpl.EXP_INT_TABLE_MAX_INDEX] = tmp[1];

                    if (i != 0) {
                        // Negative integer powers
                        AccurateMathCalc.splitReciprocal(tmp, recip);
                        EXP_INT_TABLE_A[AccurateMathImpl.EXP_INT_TABLE_MAX_INDEX - i] = recip[0];
                        EXP_INT_TABLE_B[AccurateMathImpl.EXP_INT_TABLE_MAX_INDEX - i] = recip[1];
                    }
                }
            } else {
    ...
    private static class ExpFracTable {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                EXP_FRAC_TABLE_A = new double[AccurateMathImpl.EXP_FRAC_TABLE_LEN];
                EXP_FRAC_TABLE_B = new double[AccurateMathImpl.EXP_FRAC_TABLE_LEN];
    ...
    private static class lnMant {
        ...
        static {
            if (RECOMPUTE_TABLES_AT_RUNTIME) {
                LN_MANT = new double[AccurateMathImpl.LN_MANT_LEN][];
